...
Ein vollwertiges "`Fly-by-Wire"' Steuersystem für Verkehrsflugzeuge setzt sich aus über 40 vernetzten Computern zusammen, die jeweils für die Durchführung und Überwachung der Ansteuerung einzelner Steuerflächen verantwortlich sind (siehe Abb.~\ref{fig:fcs}).
...
\begin{itemize}
\item Es bleibt nicht nur bei den konzeptionellen Erweiterungen / Ergebnissen
\item Das Verfahren wird auch als Softwarewerkzeug ASSIST (siehe Abb.~\ref{fig:assist}) realisiert
\item Ein Fachexperte kann in einer textuellen Eingabesprache das Platzierungsproblem spezifizieren
\item Die Sprache orientiert sich an den Begrifflichkeiten der Industrie, um den Einstieg und die Verwendung zu erleichtern
\item Nach der Spezifikation wird die Transformation des Problems in ein CSP und dessen Lösung automatisiert vorgenommen;
\item Die Lösungen des CSP (also die Belegungen der einzelnen Variablen) werden auch wieder rücktransformiert, so dass die technische Seite der Problemlösung vor dem Anwender vollständig verborgen ist
\end{itemize}
...
\begin{itemize}
\item Demonstration des Einsatzes dieses Werkzeugs mit Hilfe von zwei Fallbeispielen
\item FCS von Liebherr Aerospace "= Schwerpunkt war hier ...
\item Steuersystem für Raumfahrzeuge 
\end{itemize}
